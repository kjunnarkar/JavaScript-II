...
let firstNamesAllCaps = [];
  firstNamesAllCaps = runners.map(function(firstName) {
...
let runnersLargeSizeShirt = [];
  runnersLargeSizeShirt = runners.filter(function(largeShirtList) {
  return largeShirtList.shirt_size === "L";
  });
...
let ticketPriceTotal = 0;
  ticketPriceTotal = runners.reduce(function(accumulator,revenue) {
  return accumulator + revenue.donation;
  }, 0);
...
// Problem 1 Target high paying donors (> 200) for future fundraising events.
let highDonors = [];
  highDonors = runners.filter(function(contribution) {
  return contribution.donation > 200;
  });
console.log(highDonors);

// Problem 2 Get Email list of all donors to send "thank you" email.
let thankYou = [];
  runners.forEach(function(emailList) {
    return thankYou.push(`${emailList.email}`);
  });
console.log(thankYou);

// Problem 3 Figure out who needs 2XL shirts.
let shirtsXL = [];
  shirtsXL = runners.filter(function(shirts) {
  return shirts.shirt_size === "2XL";
  });
console.log(shirtsXL);
